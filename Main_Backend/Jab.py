...
def initial_stance(lmlist):
    if len(lmlist) >= 16:  # Check if lmlist contains at least 16 landmarks
        wrist = lmlist[15][2]  # Y-coordinate of the wrist landmark
        elbow = lmlist[14][2]  # Y-coordinate of the elbow landmark
        return wrist < elbow
    else:
        return False

cap = cv2.VideoCapture('Videos\Sample.'
                       'mp4')
    ...
    if not initial_stance_detected:
        ...
        if not final_pose_detected:
            if initial_stance(lmlist):  # Require the user to follow the initial stance before detecting the final pose
                if angle_threshold(lmlist, [11, 13, 15], [12, 14, 16], [13, 11, 23], [14, 12, 24], [23, 25, 27], [11, 23, 25], [24, 26, 28], [12,24,26], thresholds=thresholds):
            ...
            if not cooldown_active:
                ...
                if not initial_stance(lmlist):
                    ...
                    angles = process_pose(lmlist, [11, 13, 15], [12, 14, 16], [13, 11, 23], [14, 12, 24], [23, 25, 27], [11, 23, 25], [24, 26, 28], [12,24,26])
                    ...
                    interpolated_angles = [int(np.interp(angle, src, dst)) for angle, src, dst in zip(angles,
                                                                                                          [[50, 175], [80, 50],
                                                                                                           [40, 105], [10, 40], [195, 203], [150, 145], [180, 178], [195, 198]],
                                                                                                          [[0, 100], [100, 0],
                                                                                                           [0, 100], [100, 0], [0,100], [100, 0], [100,0], [0,100]])]
